...
func main() {
	http.HandleFunc("/", spotsReqHandler)
	log.Fatal(http.ListenAndServe(SERVER, nil))
}

func spotsReqHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		http.Error(w, "Wrong radius", http.StatusBadRequest)
		return
	}

	latitude, err := strconv.ParseFloat(r.URL.Query().Get("latitude"), 64)
	if err != nil {
		http.Error(w, "Wrong latitude", http.StatusBadRequest)
		return
	}

	longitude, err := strconv.ParseFloat(r.URL.Query().Get("longitude"), 64)
	if err != nil {
		http.Error(w, "Wrong longitude", http.StatusBadRequest)
		return
	}

	spots, err := getSpots(latitude, longitude, radius, locationType)
	if err != nil {
...
func getSpots(latitude, longitude, radius float64, locationType string) ([]Spot, error) {
